...
//edited for part 1.2
/*the amound of the sched queues*/
#define MINIMUM 0
#define MAXIMUM 10

/*the counter for the sched queue boost*/
...
//edited for part 1.1
PTCB* spawn_ptcb(PCB* cur_pcb,Task task, int argl, void* args)
{
	 //Allocate space for new PTCB
    ...
    rlist_push_back(&cur_pcb->ptcb_list, &ptcb->ptcb_list_node);
	return ptcb;
}
...
//edited for part 1.2
rlnode SCHED[MAXIMUM]; /* The scheduler queue */
...
static void sched_queue_add(TCB* tcb)
{
	...
	rlist_push_back(&SCHED[tcb->pr_level], &tcb->sched_node);
...
static TCB* sched_queue_select(TCB* current)
{
	...
	do{
		...
		i++;
	}while(i<MAXIMUM && next_thread == NULL);
...
//edited for part 1.2
/**************************************** boost ****************************************/
void prboost(TCB* current){
	TCB* thread ;
	if(yield_count >= 50 ){
		int j = MINIMUM;
		do{
			rlnode* sel = rlist_pop_front(&SCHED[j+1]);

			thread = sel->tcb; /* When the list is empty, this is NULL */
			if (thread != NULL)
			{
				rlist_push_back(&SCHED[j], &thread->sched_node);
			}
			else j++;
...
//edited for part 1.2
/************* This is the priority level for the current TCB block queue *************/
void priority_finder(TCB* current){

	if(&current->curr_cause == SCHED_QUANTUM ) {  		//if the cause is SCHED_QUANTUM then we lower down the priority
		 if(current->pr_level < MAXIMUM-1) {
			current->pr_level ++;
		}
		
		 current->c_sum = 0;
	}
	else if(current->curr_cause == SCHED_IO ) {			//if the cause is SCHED_IO then we make the priority higher
		if(current->pr_level > MINIMUM){
		 	current->pr_level --;
		 }

		current->c_sum = 0;
	}
	else if(current->curr_cause == SCHED_MUTEX ) {
		if(current->c_sum >= 1){						//if the cause is SCHED_MUTEX and we had seen SCHED_MUTEX in last and current cause 10 times or more already then we make the priority higher
			if(current->pr_level > MINIMUM) {
				current->pr_level --;
				current->c_sum = 0;
			}
		}
		else if(current->last_cause == SCHED_MUTEX ){	//if the cause is SCHED_MUTEX in last and current cause then we lower down the priority
			if(current->pr_level < MAXIMUM-1) {
				current->pr_level ++;
				current->c_sum ++;
			}
	...
	else if(current->curr_cause == ( SCHED_PIPE || SCHED_POLL || SCHED_IDLE || SCHED_USER ) ) {
		current->c_sum = 0;
	}
	else {  											//if the cause is SCHED_PIPE or SCHED_POLL or SCHED_IDLE or SCHED_USER or something else then we keep the priority as it is
		current->c_sum = 0;
	}
